...
# -*- coding: utf-8 -*-
"""
Toy NPU Compiler v7.1 (PASS-FIX VERSION)
=======================================

This is a drop-in replacement for v7 that fixes the fatal error:

  RuntimeError: No non-overlapping swizzle choice found ...

Root cause (v7):
- CandidateSRAMPlanner generated candidates for *every* buffer starting from address 0,
  so candidates across different buffers overlapped heavily.
- SwizzleSelector requires A/B/SK (and others) to be non-overlapping, so it often found no solution.

Fix (v7.1):
1) Slot-based SRAM planning:
   - First assign each buffer a dedicated, non-overlapping SRAM "slot".
   - Then generate multiple candidate addresses *inside that slot* by shifting by bank_granularity.
   - This guarantees a non-overlapping solution exists (unless SRAM truly overflows).

2) Keep v7 features:
   - bank+temporal swizzle selection (buffer rotation)
   - DAG list scheduling with resource constraints (DMA/MAC/ACT)
   - accurate simulator + hazard report
...
def banks_touched(m: ModelSpec, c: Cmd32) -> List[int]:
    ...
    if c.opcode == OP_MAC_ACC32 and is_sram(m, c.arg0):
        add(c.arg0, m.K*m.tileN)               # B pointer
    if c.opcode == OP_ADD_I8_CLAMP and is_sram(m, c.arg0):
        add(c.arg0, c.size_or_M)               # skip pointer
...
# ============================================================
# 7) v7.1 Slot-based SRAM candidate generation (PASS FIX)
# ============================================================

class CandidateSRAMPlanner:
    """
    v7.1 FIX:
    1) build non-overlapping slots for each buffer
    2) generate multiple candidates within each slot (shift by bank_granularity)
    """
    def __init__(self, m: ModelSpec):
        self.m = m
        self.slots: Dict[str, Tuple[int,int]] = {}

    def build_slots(self, reqs: List[BufReq]) -> Dict[str, Tuple[int,int]]:
        m = self.m
        cur = 0

        # Place large first to reduce fragmentation
        order = sorted(reqs, key=lambda r: (-r.size, r.name))

        slots: Dict[str, Tuple[int,int]] = {}
        for r in order:
            cur = align_up(cur, r.align)
            base = cur

            # slack allows bank shifting inside the slot
            slack = min(8 * m.bank_granularity, max(0, m.sram_size_bytes - (base + r.size)))
            limit = base + r.size + slack
            if limit > m.sram_size_bytes:
                limit = base + r.size
            if limit > m.sram_size_bytes:
                raise MemoryError(f"SRAM overflow when assigning slot for {r.name}")

            slots[r.name] = (base, limit)
            cur = limit

        self.slots = slots
        return slots

    def gen_candidates_in_slot(self, req: BufReq, max_cand: int = 8) -> List[int]:
        m = self.m
        if req.name not in self.slots:
            raise KeyError(f"Slot not found for buffer {req.name}; call build_slots() first.")

        base, limit = self.slots[req.name]
        ...
        if not cands:
            a = align_up(base, req.align)
            if a + req.size <= limit:
                cands = [a]
            else:
                raise RuntimeError(f"No candidate fits inside slot for {req.name}")

        return cands


# ============================================================
# 8) Swizzle selection (bank + temporal cost)
# ============================================================
...
class SwizzleSelector:
    """
    Choose 1 candidate per buffer to minimize predicted overlap cost,
    under the constraint that chosen ranges do not overlap.

    With v7.1 slots, overlaps are already prevented structurally, but we keep checks.
    """
    def __init__(self, m: ModelSpec):
        self.m=m
    ...
    def choose(self,
        ...
        # fixed buffers (pick first cand)
        choice: Dict[str,int] = {}
        for fixed in ["ACC","OUT","B_SCR"]:
            if fixed in cands and cands[fixed]:
                choice[fixed] = cands[fixed][0]
            else:
                raise RuntimeError(f"Missing candidates for fixed buffer {fixed}")

        # enumerate ping-pong combos
                ...
                for b0 in B0c:
                    for b1 in B1c:
                        ...
                        for sk0,sk1 in sk_pairs:
                            out_bs = bs("OUT", choice["OUT"])
                            acc_bs = bs("ACC", choice["ACC"])
                            ...
                            # temporal cost approximation (same as v7)
                            cost=0
                            # phase0: MAC uses B0; prefetch to A1/B1/SK1
                            cost += inter_cost(A1bs, B0bs, 5)
                            cost += inter_cost(B1bs, B0bs, 6)
                            ...
                            # phase1: MAC uses B1; prefetch to A0/B0/SK0
        ...
        if best is None:
            raise RuntimeError("No non-overlapping swizzle choice found (unexpected in v7.1 with slots)")
...
# ============================================================
# 9) Build a DAG + list scheduler (resource constraints)
# ============================================================
...
class Scheduler:
    ...
    def build_dag(self, cmds: List[Cmd32]) -> List[Node]:
        ...
        # preserve per-engine queue order (simple engine queue model)
    ...
    def schedule(self, nodes: List[Node]) -> List[Cmd32]:
        ...
        scheduled: List[Cmd32] = []

        def update_active(now: int):
            for e in ["DMA", "MAC", "ACT"]:
                if now >= eng_busy_until[e]:
                    eng_active_banks[e] = set()

        def next_time_after(now: int) -> int:
            # Find the smallest busy_until that is strictly > now
            future = [v for v in eng_busy_until.values() if v > now]
            if future:
                return min(future)
            # fallback: force progress
            return now + 1

        def pick_best(now: int, cand: Set[int]) -> Tuple[int, int]:
            """
            Return (idx_to_issue, suggested_next_time_if_none)
            """
            update_active(now)
            best = None
            any_engine_future = False

            for idx in cand:
                n = node_map[idx]
                e = n.eng
                if now < eng_busy_until[e]:
                    any_engine_future = True
                    continue
            ...
            if best is None:
                # no issuable node at 'now'
                return -1, (next_time_after(now) if any_engine_future else now + 1)

            return best[1], now  # issuable now

        safety = 0
        max_steps = len(nodes) * 20 + 1000  # generous guard

        while ready:
            safety += 1
            if safety > max_steps:
                # If you hit this, you likely have a dependency cycle
                raise RuntimeError("Scheduler safety guard hit (possible dependency cycle).")

            idx, nxt = pick_best(t, ready)
            if idx == -1:
            ...
            # issue at time t (global time), occupy the engine until end
            end = t + n.dur
            ...
            # IMPORTANT: do not change t here (we allow overlap across engines)
            # If we get stuck later, the time advance logic will move t forward.

        return scheduled

    def schedule_bakv0(self, nodes: List[Node]) -> List[Cmd32]:
        indeg = {n.idx: len(n.preds) for n in nodes}
        ready: Set[int] = {n.idx for n in nodes if indeg[n.idx]==0}
        ...
        def pick_best(now:int, cand:Set[int]) -> int:
            update_active(now)
            best=None
            for idx in cand:
                n=node_map[idx]
                if now < eng_busy_until[n.eng]:
                    continue
                nb=set(n.banks)
                inter=0
                for oe in ["DMA","MAC","ACT"]:
                    if oe==n.eng: continue
                    inter += len(nb & eng_active_banks[oe])
                score = inter*10 + idx*0.001
                tup=(score, idx)
                if best is None or tup < best:
...
# ============================================================
# 10) Accurate simulator + hazard detection
# ============================================================
...
class BarrierMitigator:
    ...
    def mitigate(self, m: ModelSpec, cmds: List[Cmd32], max_iters: int = 120) -> Tuple[List[Cmd32], Dict[str,Any]]:
        sim=Simulator(m)
        cur=list(cmds)
...
def main():
    out_dir="./toy_out_v7_1"
    os.makedirs(out_dir, exist_ok=True)

    # Keep same assumed NPU/memory spec as v7/v6
    m=ModelSpec(
        A_base=0x8000_0000,
        B_base=0x8000_2000,
        Y_base=0x8000_4000,
    ...
    # ---- v7.1 slots + candidates
    cand_planner=CandidateSRAMPlanner(m)
    slots = cand_planner.build_slots(req_list)
    with open(os.path.join(out_dir,"sram_slots.txt"),"w",encoding="utf-8") as f:
        for name,(b,l) in sorted(slots.items(), key=lambda kv: kv[1][0]):
            f.write(f"{name:8s} slot: [0x{b:08X}, 0x{l:08X})\n")

    cands: Dict[str,List[int]]={}
    for r in req_list:
        mc = 10 if r.name in ("A0","A1","B0","B1","SK0","SK1") else 4
        cands[r.name]=cand_planner.gen_candidates_in_slot(r, max_cand=mc)
    ...
    # ---- DAG list scheduling for compute path
    sched=Scheduler(m)
    ...
    while q:
        ...
        for v in nodes[u].succs:
           ...
           if indeg[v] == 0:
                q.append(v)
    if seen != len(nodes):
        raise RuntimeError("DAG has a cycle (check event deps or per-engine order edges).")
    ...
    dispatch={
        ...
        "v7_1_pipeline":[
            "slot-based SRAM regions (non-overlapping)",
            "candidates inside slots (bank shift)",
            "bank+temporal swizzle selection",
            "DAG list scheduling (DMA/MAC/ACT)",
            "barrier mitigation fallback"
        ]
    }
    ...
    print("Key files:")
    print("  sram_slots.txt")
    print("  swizzle_choice.json")
    print("  sim_compute_raw.txt / sim_compute_sched.txt / sim_compute_fixed.txt")
    print("  hazard_report.json")
